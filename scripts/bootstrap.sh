...
CHEZMOI_SOURCE_DIR="$DEFAULT_SOURCE_DIR"
...
Main modes:
  --source-dir DIR         Use local starter-kit directory (default: parent of this script)
  --repo-url URL           Use remote chezmoi git repository instead of local starter-kit
...
Examples:
  ...
  ./bootstrap.sh --repo-url git@github.com:Ephraimus/drifter.git --repo-branch main
USAGE
}
...
install_packages() {
  ...
  case "$pm" in
    apt)
      ...
      run_as_root apt-get update -y || warn "apt-get update encountered errors, but we will try to continue"
...
apply_local_starter_kit() {
  [ -d "$CHEZMOI_SOURCE_DIR" ] || die "Starter-kit directory not found: $CHEZMOI_SOURCE_DIR"
