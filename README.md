...
## Какой Shell Выбирать

- `bash` — основной вариант для серверов, SSH и консервативных Linux-окружений; это базовый путь для `drifter`.
- `zsh` — нормальный вариант, если он уже является твоим привычным интерактивным shell и ты не хочешь менять workflow.
- `fish` — лучший вариант для локальной машины или WSL, если нужен более удобный интерактивный UX по умолчанию.

Практически:

- серверы и remote hosts: оставить `bash`;
- локальная машина или WSL: можно включать `fish`;
- если уже живёшь в `zsh` и всё устраивает: `drifter` можно использовать и без смены login shell.

`--set-login-shell` делает best-effort попытку через `chsh`, но не гарантирует результат. Для успеха shell должен быть установлен, присутствовать в `/etc/shells`, а `chsh` должен отработать без ошибки. Если шаг не сработал, bootstrap печатает точную команду для ручного запуска.
